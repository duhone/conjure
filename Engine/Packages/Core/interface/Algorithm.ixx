...
export namespace CR::Engine::Core {
	inline void UnorderedRemove(std::ranges::range auto& a_container, std::input_iterator auto& a_iterator) {
		*a_iterator = *a_container.back();
		a_container.pop_back();
	}

	inline std::size_t SortedInsert(std::ranges::range auto& a_container, const std::regular auto& a_value,
