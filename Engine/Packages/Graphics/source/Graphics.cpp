...
import CR.Engine.Graphics.Sprites;
...
#pragma pack(pop)
	...
	void FindDevice(cegraph::Context& context) {
		...
		vkGetPhysicalDeviceQueueFamilyProperties(physicalDevices[foundDevice], &numQueueProps,
		                                         queueProps.data());
		...
		cegraph::ClearStruct(featuresRobust);
		featuresRobust.pNext = nullptr;

		VkPhysicalDeviceVulkan14Features features14;
		cegraph::ClearStruct(features14);
		features14.pNext = &featuresRobust;

		VkPhysicalDeviceVulkan13Features features13;
		cegraph::ClearStruct(features13);
		features13.pNext = &features14;

		VkPhysicalDeviceVulkan12Features features12;
		cegraph::ClearStruct(features12);
		features12.pNext = &features13;

		VkPhysicalDeviceVulkan11Features features11;
		cegraph::ClearStruct(features11);
		features11.pNext = &features12;

		VkPhysicalDeviceFeatures2 features;
		cegraph::ClearStruct(features);
		...
		CR_ASSERT(features11.uniformAndStorageBuffer16BitAccess,
		          "Require support for 16 bit types in buffers");
	...
	void BuildDevice(cegraph::Context& context) {
		VkPhysicalDeviceMemoryProperties memProps;
		...
		cegraph::ClearStruct(requiredFeatures);
		...
		vkGetDeviceQueue(context.Device, m_presentationQueueIndex, presentationQueueIndex,
		                 &m_presentationQueue);
	...
	bool CreateSwapChain(const cegraph::Context& context) {
		...
		CR_LOG("Min image count: {} Max image count: {}", surfaceCaps.minImageCount,
...

		{
			...
			cegraph::ClearStruct(msaaCreateInfo);
		...
		cegraph::ClearStruct(swapCreateInfo);
...
void cegraph::Initialize(GLFWwindow* a_window) {
	...
	win32Surface.hwnd      = glfwGetWin32Window(m_window);
	...
	m_commandPool = cegraph::CommandPools::Create(context.GraphicsQueueIndex);
...
bool cegraph::Render() {
	...
	cegraph::CommandPools::ResetAll(m_commandPool);
	auto commandBuffer = cegraph::CommandPools::Begin(m_commandPool);
