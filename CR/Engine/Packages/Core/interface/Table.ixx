...
export namespace CR::Engine::Core {
	...
	class Table {
	  ...
	  public:
		...
		uint16_t insert(t_primaryKeyF&& a_key, t_columnsF&&... a_row)
		    requires !std::same_as<t_primaryKey, std::string>;
		template<typename... t_columnsF>
		uint16_t insert(const std::string_view a_key, t_columnsF&&... a_row)
		    requires std::same_as<t_primaryKey, std::string>;
		// row will be default constructed if not standard layout, otherwise uninitialized
		template<typename t_primaryKeyF>
		uint16_t insert(t_primaryKeyF&& a_key)
		    requires !std::same_as<t_primaryKey, std::string>;
...
template<uint16_t c_maxSize, std::regular t_primaryKey, typename... t_columns>
template<typename t_primaryKeyF, typename... t_columnsF>
inline uint16_t CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::insert(t_primaryKeyF&& a_key,
                                                                                       t_columnsF&&... a_row)
    requires !std::same_as<t_primaryKey, std::string>
{
...
template<typename... t_columnsF>
inline uint16_t
    CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::insert(const std::string_view a_key,
                                                                           t_columnsF&&... a_row)
    requires std::same_as<t_primaryKey, std::string>
{
...
inline uint16_t CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::insert(t_primaryKeyF&& a_key)
    requires !std::same_as<t_primaryKey, std::string>
{
	CR_ASSERT_AUDIT(m_lookUp.find(a_key) == std::end(m_lookUp),
	                "Tried to insert, but row already exists with this key {}", m_tableName);

	uint16_t unusedIndex = FindUnused();
	CR_ASSERT_AUDIT(unusedIndex != c_maxSize, "Ran out of available rows in table {}", m_tableName);
...
inline uint16_t
    ...
    requires std::same_as<t_primaryKey, std::string>
{
	CR_ASSERT_AUDIT(m_lookUp.find(std::string(a_key)) == std::end(m_lookUp),
	                "Tried to insert, but row already exists with this key {}", m_tableName);
...
inline const t_value&
	...
	CR_ASSERT_AUDIT(a_index != c_unused && m_used[a_index], "asked for an unused row in table {}",
	                m_tableName);
...
inline void CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::erase(uint16_t a_index) {
	CR_ASSERT_AUDIT(a_index != c_unused && m_used[a_index], "tried to delete an unused row in table {}",
	                m_tableName);
...
CR::Engine::Core::Table<c_maxSize, t_primaryKey, t_columns...>::Iterator<t_columnsSubset...>::Iterator(
    Table& a_table, uint16_t a_index) : m_table(a_table), m_index(a_index) {
