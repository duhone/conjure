...
void ceassets::Partition::BuildFolder(const fs::path& a_rootPath, const fs::path& a_currentPath,
	...
	for(const auto& dirEntry : fs::directory_iterator(a_currentPath)) {
		...
		if(dirEntry.is_regular_file()) {
			...
			a_currentFolder.DebugFiles.emplace_back(std::move(pathToHash));
			...
			a_currentFolder.FilePaths.emplace_back(dirEntry.path());
...
void ceassets::Partition::Load(const fs::path& a_subFolder, std::string_view a_extensionFilter,
                               LoadCallbackT a_loadCallback) {
	const Folder* currentFolder = &m_rootFolder;
	for(const auto& pathPart : a_subFolder) {
		auto found = std::ranges::find(currentFolder->FolderNames, pathPart.string());
		CR_ASSERT(found != currentFolder->FolderNames.end(), "Invalid asset subFolder {}",
		          a_subFolder.string());
		auto newFolderIndex = std::distance(currentFolder->FolderNames.begin(), found);
